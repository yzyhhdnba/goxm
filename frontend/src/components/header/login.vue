  ...
  setup(props, { emit }) {
    ...
    const checkUserName = async () => {
      ...
      try {
        ...
        // The backend returns { available: bool }
    ...
    const toRegister = () => {
      ...
      if (dialog) dialog.style.height = "460px";
    };
    
    const toLogin = () => {
      isLoginPage.value = false;
      const dialog = document.querySelector(".dialog");
      if (dialog) dialog.style.height = "524px";
    };
    ...
    // login 对应文档“登录弹窗：从登录请求到本地会话落盘”。
    // 这里把“调用登录接口 -> 保存 token -> 拉取当前用户信息”串成了一个前端闭环。
    const login = async () => {
      ...
      try {
        ...
        try {
            ...
            localStorage.setItem('userInfo', JSON.stringify({ ...userRes, userId: userRes.id }));
        } catch (e) {
    ...
    const register = async () => {
      ...
      try {
        ...
        ElNotification({
          title: "注册成功",
          message: h("p", { style: "color: green" }, "注册成功，请去登录"),
        });
        isLoginPage.value = true;
      } catch (err) {
        console.error(err);
    ...
    const bluring = () => {
      const left = document.querySelector(".buttom-left");
      const right = document.querySelector(".buttom-right");
      // Note: Adjusting require with context is tricky in script replacement, but using require should still work since it's processed by webpack.
      if (left) left.style.backgroundImage = 'url(' + require("../../assets/image/22_open.png") + ')';
      if (right) right.style.backgroundImage = 'url(' + require("../../assets/image/33_open.png") + ')';
    };
    
    const focusing = () => {
      const left = document.querySelector(".buttom-left");
      const right = document.querySelector(".buttom-right");
      if (left) left.style.backgroundImage = 'url(' + require("../../assets/image/22_close.png") + ')';
      if (right) right.style.backgroundImage = 'url(' + require("../../assets/image/33_close.png") + ')';
    };
