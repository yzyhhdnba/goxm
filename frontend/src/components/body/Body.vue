...
export default {
  ...
  data() {
    return {
      swiperList: [
        ...
        { title: '精彩内容', pic: 'https://uploadstatic.mihoyo.com/contentweb/20210719/2021071917531190280.jpg' },
        { title: '发现更多', pic: 'https://uploadstatic.mihoyo.com/contentweb/20210719/2021071917534678902.jpg' },
      ],
      swiperList_r: [],
      animationList: [],
  ...
  async created() {
    try {
      // 获取热门推荐右侧 6 条
      const resRecommend = await VideoAPI.getRecommend({ limit: 6 });
      if (resRecommend && resRecommend.items) {
        this.swiperList_r = resRecommend.items;
      }
      this.swiperList_rFlag = true;

      // 获取下方推荐主列表 8 条或更多
      const resFeed = await VideoAPI.getRecommend({ limit: 8 });
      if (resFeed && resFeed.items) {
        this.animationList = resFeed.items;
      }
  ...
  methods: {
    jumpPath(title) {
        console.log("Jumping to banner path:", title);
    }
  }
}
</script>
