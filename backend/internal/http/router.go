...
func NewRouter(cfg *appconfig.Config, gormDB *gorm.DB, redisClient *appredis.Client) *gin.Engine {
	...
	videoHandler := video.NewHandler(video.NewService(video.NewRepository(gormDB), socialRepo, mediaStorage))
	commentHandler := comment.NewHandler(comment.NewService(comment.NewRepository(gormDB)))
	...
	adminHandler := admin.NewHandler(admin.NewService(admin.NewRepository(gormDB), accountRepo))
